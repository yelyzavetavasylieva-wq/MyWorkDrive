...
- **Add new share** wizard (`src/pages/AddSharePage.jsx` + `src/pages/addshare/`): 6 steps — Storage Type → Storage Settings → Share Details → Features → Users & Groups → Review & Confirm. Storage Settings is **provider-specific** (schema in `src/data/wizard.js` → `STORAGE_SETTINGS`); switching provider resets entered settings; validation is gating-based (the Next button is disabled until each step's requirements are met); creation is in-memory via `src/store/SharesContext.jsx` (`addShare`). Each step has its own URL (nested routes under `/shares/new`: index=Storage Type, then `/storage`, `/details`, `/features`, `/users`, `/review`) so usability tools like Maze can track each screen; `AddSharePage` is the layout that owns wizard state (persists across step navigation) and renders each step via `<Outlet>`. `index.html` carries the Maze universal snippet (loaded app-wide) and `vercel.json` adds an SPA rewrite so deep links work on Vercel.
